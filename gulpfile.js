...
gulp.task('sass', function() {
    gulp.src('./week3/scss/*.scss')
        .pipe(sass().on('error', function(err){
            reportError(err);
        }))       
        .pipe(gulp.dest('./week3/css'))
        .pipe(connect.reload());
});

gulp.task('js', function() {
    gulp.src('./week3/js/*.js')
        .pipe(connect.reload());
});

gulp.task('watch', function() {
    //gulp.watch(['./week3/scss/*.scss'], ['sass']);
    gulp.watch(['./week3/*.html'], ['html']);
    gulp.watch(['./week3/js/*.js'], ['js']);
});

gulp.task('minifyCss', function() {
    return gulp.src('./week3/scss/*.scss')
